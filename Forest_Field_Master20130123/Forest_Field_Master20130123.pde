...
int closeCount;//count how many times performers have come close
int farCount;//count how many time performers have been far - closenessCount can't increase 
...
//**MAIN HEADER-------------AUDIO------------**\\
  ...
  sineG = new SineWave(100, off, out.sampleRate());//1st sine wave - frequency between G and an A
  sineC = new SineWave(270, off, out.sampleRate());//2nd sine wave - frequency between C and D
  sineF = new SineWave(175, off, out.sampleRate());//3rd sine wave - frequency ~F
...
//*MAIN HEADER -----------AUDIO---------------******\\  
 ...
 //**Use the closeCount to control which notes are played
  ...
  if (closeCount==5)closeCount=2;
  if (farCount==4)farCount=1;
