...
public class ForestField_master_20121117 extends PApplet {
...
public void setup() {
  ...
  sine = new SineWave(0, 0.5f, out.sampleRate());
  sine1= new SineWave(0, 0.5f, out.sampleRate());
  sine2= new SineWave(0, 0.5f, out.sampleRate());
  sine3= new SineWave(0, 0.5f, out.sampleRate());
...
public void draw()
{
  ...
  // println (val);
  //println (average);
  magnitude = (1.1f-  (.1f +    ((audio-36) *  (1-.1f)/ (80-36)))  ); //"magnitude" is the audio control - it is based on a mapping of average to a range that makes sense for volume control (.1-1)
  //println (magnitude);

  if (magnitude > 1)
    satisfied.setVolume (0);//if 'magnitude' (audio control) is >1, then the volume of the "satisfied" track is set to 0
  if (magnitude > 0)  
    satisfied.setVolume ((magnitude)*2.2f);//if 'magnitude' (audio control) is >1, then the volume of the "satisfied" track is set to 0
  ...
  noFill();
  ellipse(100, 100, val*9.5f, val*9.5f);
  fill(255);
  noStroke();
  ellipse(100, 100, 350, 350);
  fill(0);
  text(val, 1.2f*width/4+5, 2.5f*height/4-10, h+10);
...
public void branch(float h) {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--present", "--bgcolor=#666666", "--stop-color=#cccccc", "ForestField_master_20121117" });
  }
}
