...
int onThreshold = 10;//set the threshold that the "On" Counter has to reach before the volume increments
int offThreshold = 10;//set the threshold that the "Off" Counter has to reach before the volume decrements

float onIncrement = .01;//amount volume is increased per near time step
float offIncrement = .01;//amount volume is decresead per away time step
float volMax = 10.0;//maximum volume
float volMin = 0.0;//minimum volume

float vol;//This is the amplitude (functionally volume) used in the synthesizer

AudioOutput out;//audio output
SineWave sine;//audio sine wave
...
//*************SET UP THE SKETCH*********\\
void setup(){
   size(1100, 768);//set the size to 1100 px wide and 768 high
...
//**MAIN HEADER-------------AUDIO------------**\\
  ...
  sine = new SineWave(440, vol, out.sampleRate());
...
if(average<=36){  //if rssi is at its lowest value 
      ...
      if(vol>=volMax){    //if the volume is maxxed out
          ...
          sine.setAmp(vol);//set the amplitude of the sine wave synth to the new volume
        }
